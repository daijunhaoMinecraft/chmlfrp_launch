...
try:
    ...
    notification.notify(title='出现错误', message=f'{e}', app_icon=None, timeout=10)
    sys.exit()
os.system("cls")
...
notification.notify(title = f"隧道正在启动--{usertunnel_info[get_usertunnel_select]['name']}",message = f"隧道ip地址:{usertunnel_info[get_usertunnel_select]['ip']}",app_icon = None,timeout = 10)
sys.exit()